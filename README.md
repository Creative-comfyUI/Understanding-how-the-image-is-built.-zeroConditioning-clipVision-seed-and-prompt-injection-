...
That means the prompt apply in output 8 doesn't create image but give variation of the image referece as image 2 and 3 are different. Output 8 create variation 

Let unmuted the prompt and the same prompt to output 8 to see if we have a different image that what we have when we put now text in the prompt 8 
